...
<div>
    <ul :class="`nav nav-tabs ${navClass}`">
        <li :class="{'nav-item':true,'step-doing': tab.isActive, 'step-done':tab.isDone }" v-for="(tab,tabIndex) in tabs" v-bind:key="tab.name">
            <a :class="{
                'nav-link':true,
                'disabled':topNavDisabled
            }" href="#" @click.prevent="clickedTab(tabIndex)"  v-if="tab.type!='done'">
                <img :src="tab.icon" class="my-3">
                <h2>{{tab.name}}</h2>
    ...
    <div :class="`wizard-buttons ${navClass}`" v-if="!(lastStepEnd && isCompleted)">
        <button type="button" class="mr-1 btn btn-primary" v-if="currentActive > 0 && currentActive<totalTabs" @click="previousTab()">
            {{prev}}
        </button>
        <button type="button" class="btn btn-primary" :disabled="currentActive > totalTabs - 1" @click="nextTab()"  v-if="currentActive<totalTabs">
            {{next}}
        </button>
    </div>
</div>
